...
class ResPartner(models.Model):
    ...
    def _compute_attendance_stats(self):
        for partner in self:
            ...
            partner.present_count = len(records.filtered(lambda r: r.attendance_status == 'present'))
            partner.absent_count = len(records.filtered(lambda r: r.attendance_status == 'absent'))
            partner.late_count = len(records.filtered(lambda r: r.attendance_status == 'late'))
            partner.excused_count = len(records.filtered(lambda r: r.attendance_status == 'excused'))
            
            if partner.total_attendance_records > 0:
                partner.attendance_rate = (partner.present_count / partner.total_attendance_records) * 100
    ...
    def _compute_last_activities(self):
        for partner in self:
            # Dernière présence
            last_attendance = partner.attendance_record_ids.sorted('check_in_time', reverse=True)
            partner.last_attendance_date = last_attendance[0].check_in_time if last_attendance else False
            
            # Dernière session enseignée
            last_session = partner.taught_session_ids.sorted('start_datetime', reverse=True)
            partner.last_session_taught = last_session[0].start_datetime if last_session else False
